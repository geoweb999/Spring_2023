...
// Create a map of <State, List<Client>> via stream
// Create another map of <Client, totalSpend> via stream
// Create another map of <State, Client>> via stream using two previously created maps
// Verify this map produces the correct answers
// Write a single pipeline statement based on 3 maps above by pasting in Map1 to Map3 
// then inserting Map2 into max
		
//		Map<String, List<Client>> clientsByState = clientList.stream()
//			    .collect(Collectors.groupingBy(client -> client.getAddress().getState()));
//		
//		Map<Client, Double> clientOrderTotals = clientList.stream()
//			    .collect(Collectors.toMap(
//			        client -> client,
//			        client -> client.getOrders().stream().mapToDouble(Order::getTotal).sum()
//			    ));
//
//		Map<String, Client> stateTopClients = clientsByState.entrySet().stream()
//			    .collect(Collectors.toMap(
//			        Map.Entry::getKey,
//			        entry -> entry.getValue().stream()
//			            .max(Comparator.comparingDouble(client -> clientOrderTotals.get(client)))
//			            .orElse(null)
//			    ));
		
		Map<String, Client> stateTopClients = clientList.stream()
			    .collect(Collectors.groupingBy(client -> client.getAddress().getState())).entrySet().stream()
			    	.collect(Collectors.toMap(
			    			Map.Entry::getKey,
			    			entry -> entry.getValue().stream()
			    				.max(Comparator.comparingDouble(client -> client.getOrders().stream().mapToDouble(Order::getTotal).sum()))
			    				.orElse(null)
			    ));

		System.out.println("\n"+stateTopClients.get("CA").getFirstName() + " " + stateTopClients.get("CA").getLastName());
