...
public class HomeworkM10FolderDriver {
	...
	public static void printFolderContentsWithStack(Folder topFolder) {
		...
		while(!folderStack.isEmpty()) {
			...
			int indentTimes = indentStack.pop();
			for (int i = 0; i < indentTimes; i++) {
				System.out.print(INDENT_BLOCK);
			}
			System.out.println(currentFolder.getName());

			List<FileItem> files = currentFolder.getFileList();
			if (!files.isEmpty()) {
				for (int i = 0; i <= indentTimes; i++) {
					System.out.print(INDENT_BLOCK);
				}
			...
			List<Folder> folders = currentFolder.getSubFolders();
			Collections.reverse(folders);
			for (Folder f : folders) {
				folderStack.push(f);
				indentStack.push(indentTimes + 1);
			}
		}	
	}
