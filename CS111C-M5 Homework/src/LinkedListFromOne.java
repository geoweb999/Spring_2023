...
public class LinkedListFromOne<T extends Comparable<? super T>> implements ListFromOne<T> {
	...
	public void removeAll(T item) {
		if (!isEmpty()) {
			if (size == 1) { // special case for singleton list
				...
				while (head != null && head.data.equals(item)) { // remove from the beginning 		
					head = head.next;
					size--;
				}
	...
	public void addAll(T[] items) {
		...
		for (int i = start; i < items.length; i++) {
			Node node = new Node(items[i],null);
			current.next = node;
			current = node;
			size++;
		}
	...
	public T getMax() {
		...
		while(current != null) {
			T currentElement = current.data;
			if(currentElement.compareTo(maxValue) > 0) { // currentElement is > Current Max
